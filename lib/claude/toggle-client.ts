...
export class ClaudeToggleClient {
  ...
  async chat(messages: ChatMessage[], maxTokens: number = 1024, webSearchEnabled: boolean = false): Promise<ChatMessage> {
    try {
      console.log(`Sending chat request to Claude ${webSearchEnabled ? 'with' : 'without'} Tavily tools`);
      ...
      // Make the initial request with or without tool definitions based on toggle
      const response = await this.client.messages.create({
        ...
        ...(webSearchEnabled ? { 
          tools: tavilyTools as any,
          tool_choice: { type: "auto" } 
        } : {})
      });
      
      console.log(`Claude response ${webSearchEnabled ? 'with' : 'without'} Tavily tools:`, response);
      
      // Check if Claude wants to use a tool (only possible if web search is enabled)
      const hasToolUse = response.content.some(block => block.type === 'tool_use');
      
      if (hasToolUse && webSearchEnabled) {
      ...
      console.error(`Claude chat ${webSearchEnabled ? 'with' : 'without'} Tavily error:`, error);
      ...
      throw new Error(`Claude chat ${webSearchEnabled ? 'with' : 'without'} Tavily failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
